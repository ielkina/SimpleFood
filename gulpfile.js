...
function images() {
  return src("src/img/**/*.*")
    .pipe(newer("src/img/**/*.*"))
    .pipe(size({ title: "jpg, png, svg до" }))
    .pipe(
      imagemin([
    ...
    .pipe(size({ title: "jpg, png, svg" }))
    .pipe(dest(distPath + "img/"))
    .pipe(size({ title: "webp до" }))
    .pipe(webp())
...
exports.default = parallel(fonts, html, styles, scripts, browsersync, watching); //gulp
